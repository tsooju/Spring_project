...
public class BoardController {
	...
	public String boardReadcountTop3Method() throws UnsupportedEncodingException {
		...
		return sendJson.toJSONString(); // ==>> viewResolver 뷰리졸버로 리턴함
	...
	//게시글 페이지 단위로 목록보기 요청 처리용
		...
		public String replyInsertMethod(Board reply, 
			...
			// 댓글의 댓글일 경우 board_reply_ref(참조하는 댓글번호) 값을 지정함.
			...
			if(!mfile.isEmpty()) {
				...
				if(fileName != null && fileName.length() > 0) {
					...
					// notice 객체에 첨부파일명 기록 저장하기
					 ...
					 board.setBoard_rename_filename(fileName);
				}
		...
		public String boardDeleteMethod(Board board,
				HttpServletRequest request, Model model) {
			if(boardService.deleteBoard(board) >0) {
				// 글삭제가 성공했으며, 저장폴더에 첨부파일도 삭제 처리
		...
		public String boardUpdateMethod(Board board, Model model, 
			...
			//2. 새로운 첨부파일이 있을 경우 : 게시글 첨부파일은 1개만 가능한 경우
			if(!mfile.isEmpty()) {
