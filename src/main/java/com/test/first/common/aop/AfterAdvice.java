...
public class AfterAdvice {
	...
	// private Logger logger = LoggerFactory.getLogger(AfterAdvice.class());   ==> 아래코드랑 같은 의미
	private Logger logger = LoggerFactory.getLogger(getClass());
	
	
	// Pointcut 설정에 사용되는 메소드 작성 : 내용없이 작성해야 함. 
	@Pointcut("execution(* com.test.first..*Impl.*(..))")
	public void implPointcut() {
		
	}
	
	// Pointcut 이 설정한 위치에 위빙될 어드바이스 코드 작성용 메소드
	@After("implPointcut()")
	public void finallyLog() {
		// 예외발생 상관없이 무조건 실행되는 어드바이스임
		logger.info("Impl 클래스 메소드 실행 후 작동 : " 
							+ "서비스 로직 수행이 왼료된 후 무조건 작동됨");
	}
