...
update notice set noticewriter = 'admin';
commit;
...
COMMENT ON COLUMN BOARD.BOARD_REF IS '원글번호';  -- 원글번호: 원글은 자기 번호를 가진다.
COMMENT ON COLUMN BOARD.BOARD_REF IS '참조답글번호';  -- 원글 : null, 원글의 답/댓글 : 자기번호, 답글의 답글 : 참조답글번호
